...
`trip_type=box` respects `star`; unknown `trip_type` values still price as regular. Clients never compute fares — show `fare_iqd` and "includes Box fee `surcharge_iqd`".
