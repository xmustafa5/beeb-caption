...
| Fix my car details | **Live (2026-09-13)** | `PUT /api/captain/me/vehicle` `{car_make, car_model, car_color?, car_brand_id?, car_model_id?, car_year?}` → `200 Captain`, re-graded unless an admin has pinned the star. **A PUT, not a PATCH — omitted optional fields are cleared, and omitting `car_year` drops the captain to star 1**, so pre-fill the form from the current record. `409` (with `trip_id`) while on an `accepted`/`in_progress` trip; `400` on blank make/model, a year outside 1970..=2100, or a model/brand mismatch. `car_plate` is not editable. See [Vehicle edit](#6-vehicle-edit--new-endpoints-2026-09-13) |
...
- **Override / re-grade (super_admin):** `PUT /api/captains/{id}/star` `{ star: 1|2|3 }` → `200 Captain`. Unchanged contract, but it now also **pins** the grade — an admin-set star is recorded with `star_overridden_at` and is skipped by any automatic re-grade, so a human decision is never silently undone. `star` outside 1..3 → `400`; unknown captain → `404`; non-super_admin → `403`. Publishes `beep.captain.star_changed`.
...
| `star_overridden_at` | `RFC3339 \| null` | When an admin last set `star` by hand. **Non-NULL means the grade is PINNED**: a human decided it, so it is skipped by automatic re-grading and survives a vehicle edit untouched. Null means the grade is whatever the classifier last produced. |

**Where they appear:** everywhere a `Captain` is returned: `POST /api/captains/register`, `GET /api/captains/{id}`, and the approve/reject/block/star/vehicle responses. **`GET /api/captains` list rows are different** — `CaptainRow` carries `car_brand_id`, `car_model_id` and `star_overridden_at` but **not** `star` or `car_year`; fetch the captain by id when you need the grade itself.
- **Pinning is a real cost, so prefer fixing the car.** `PUT /api/captains/{id}/star` stamps `star_overridden_at` and permanently exempts that captain from automatic re-grading. Use the vehicle-edit endpoints below to correct a *wrong description*; reserve `/star` for a grade you genuinely want frozen against the classifier.
...
**This is a PUT, not a PATCH.** The whole vehicle block is replaced and every optional field you omit is **CLEARED**. The trap that matters: **omitting `car_year` drops the captain to star 1**, because a car with no year clears no age gate. Always send the full block, pre-filled from the captain's current record.
...
**The re-grade-unless-pinned rule, stated plainly:** the star is recomputed through the **same classifier registration uses** — *unless* `star_overridden_at` is set, in which case `star` is left exactly as the admin decided and only the description changes. A captain editing their own car can therefore never silently undo a deliberate admin override. The catalog ids **are** still resolved and stored for a pinned captain (what was pinned is the *grade*, not the claim about which car this is), so lifting the pin later grades them correctly.

**Status codes:**

| Code | When |
|-|-|
| `200` | Updated (and re-graded unless pinned). Body is the full `Captain`, so read `star`, `car_brand_id`, `car_model_id` and `star_overridden_at` straight back off the response instead of re-fetching. |
| `400` | Blank `car_make`/`car_model`; `car_year` outside 1970..=2100; a `car_model_id` that is not in the catalog; or a `car_model_id` that does not belong to the `car_brand_id` you sent. A mismatched pair is **rejected, not silently stored** — storing it would grade the captain as a car they did not pick. |
...
>
> A 20 km star-1 ride, at the seeded rates: **6,000 IQD booked ahead before this change → 12,000 IQD now**, which is exactly what that ride has always cost booked immediately. The gap scales with distance — every km under the 10 km threshold doubles (250 → 500 IQD/km) and every km past it quadruples (125 → 500 IQD/km) — so a 5 km ride goes 3,250 → 4,500 and a 10 km ride 4,500 → 7,000. Star 2 and star 3 are higher again (700 and 1,000 IQD/km).
>
