...
| `rt:captain:{id}` | **two frame kinds** — offer broadcast: `{ "trip_id", "captain_id", "trip_type", "pickup_lat", "pickup_lng", "dropoff_lat", "dropoff_lng", "pickup_address", "dropoff_address", "fare_iqd", "distance_km" }` (**`trip_type` `"regular"`\|`"box"` added 2026-09-25**, additive; parcel details come from the trip-queue poll, never the recipient phone); and (**NEW 2026-09-06**) a mirror of the `trip_update` frame above. | The captain socket now actually subscribes to this channel (it never did before), so **dispatch offers arrive live over WS**, not only via the 8s queue poll and FCM. Discriminate the two: an offer has `trip_id` + `pickup_lat` and no `event`; a lifecycle frame has `"event": "trip_update"` + `id` + `status`. No key overlap. The Captain App should still rely on the durable FCM push (below) for backgrounded delivery. |
...
| `trip_accepted` | `{ trip_id }` | rider | a captain accepts the trip (Box: body "A captain is coming to pick up your box") |
| `captain_arriving` | `{ trip_id }` | rider | captain marks arrived |
| `trip_completed` | `{ trip_id }` | rider | trip completed (Box: title "Box delivered", body "Your box has been delivered") |
...
| `new_trip_in_queue` | `{ trip_id }`, or `{ room_id }` for a Nafarat room | candidate captain | a new trip is dispatched nearby, or a room nearby reaches its rider minimum (2026-09-23). A Box trip uses the same type with title "New delivery nearby" / body "A customer wants to send a box near you." (2026-09-25) |
...
## Service area, Nafarat and room rules (2026-09-23 — in code; live after the next deploy)

Three product rules, all enforced server-side so old app builds are covered too:

**1. Beep runs inside Iraq only.** These refuse any pickup, dropoff or stop outside Iraq with **`400 {"error":"outside_service_area"}`** — a bare code, no `bad request:` prefix (new `AppError::BadRequestCode`):
- `POST /api/trips`, `GET /api/trips/estimate`
- `POST /api/rider/scheduled-trips`, `PUT /api/rider/scheduled-trips/{id}` (checked on the merged coordinates — moving only a latitude can still leave Iraq)
- `POST /api/rider/trips/{id}/stops`

The border is Natural Earth 1:10m, pushed 2 km outward and simplified to 225 points (`src/common/iraq_border.rs`); the rider app checks the identical outline (`Beeb/lib/iraq-border.ts`) so it can stop the pin before any request. Clients: map the code to copy ("Beep works inside Iraq only"), never show it raw.

**2. Nafarat (Abriyah) is Baghdad-only for now — "coming soon" elsewhere.** Nafarat already only works inside Abriyah zones, and every live zone is in Baghdad; what changed is the answer. `POST /api/abriyah/join` now returns **`400 {"error":"outside_abriyah_area"}`** when the dropoff is outside every Abriyah zone or the pickup is outside every active zone (was `bad request: dropoff is not inside an Abriyah-enabled zone` / `bad request: pickup is not inside any active service zone`). Show it as "coming soon to this area". `validate-pins` is unchanged (still 200 + `valid:false`).

**3. A room goes to captains from its 2nd rider** (it used to be offered from the 1st):
- **Setting `room.min_riders_to_offer`** (seeded `2`, super-admin editable 1–6; `1` restores the old behaviour). A room is ready once it holds that many **live** riders (member trips still `matched` — a cancelled member stops counting), capped at the room's `max_riders` so a smaller room still goes out when full. One SQL function, `beep_room_min_riders_to_offer()`, reads the key for every consumer below.
- **`GET /api/captain/trip-queue`** lists a room only once it is ready. The room's pickup now comes from the earliest rider still waiting (a cancelled seed no longer supplies it).
- **Push on ready (NEW):** the join that makes a room ready alerts up to 20 nearest captains within 10 km of that rider's pickup who are approved, activated today, online, not on a trip, Abriyah-approved, and female for a women-only room — the same two signals as a regular trip: a WS frame on `rt:captain:{id}` `{ room_id, trip_id, captain_id, pickup_lat, pickup_lng, fare_iqd, rider_count }` (captain app: it is an offer-shaped nudge — refetch the queue) and FCM `new_trip_in_queue` with `data: { room_id }`. Rooms used to reach captains **only** through the 8 s queue poll.
- **Fresh wait on ready:** that same join pushes the room's `expires_at` out to `now + room_max_wait_seconds`, so a second rider joining late still leaves captains time. The `rt:room:{id}` join frame now carries `expires_at`, which the rider app already applies to its countdown.
- **`POST /api/abriyah/rooms/{id}/accept`** refuses a room that is not ready with **`409 {"error":"conflict: room is still waiting for more riders"}`** (only reachable if a rider left between the poll and the tap — drop the card, as for any 409).
- **Fixed — captains locked out after one room:** a `dispatched` room never leaves that status, so `accept` answered every later room with `409 captain already has an active room assignment`. A dispatched room now only blocks its captain while one of its trips is `accepted`/`in_progress`.
- **Fixed — expired-room race:** a room past its deadline kept its zone's slot until the 30 s sweep; a join into that zone 500'd, and its own riders were refused a new room. Joins now run the sweep first (same work: expire, cancel member trips with `system_timeout`, notify riders).
...
|-|-|-|
| Car class (star) picker | **Updated 2026-09-13** | `GET /api/trips/estimate?...&star=1\|2\|3` now returns **three different fares** (500 / 700 / 1000 IQD/km) — they used to be identical, so remove any assumption that they match. Star 2/3 requests are no longer stranded: dispatch widens to higher-graded captains after 20 s, and the rider still pays the tier they booked. See [Captain Car Star Class](#captain-car-star-class-live) |
...
| Scheduled / multi-stop | 12 **Live**; star + repricing **2026-09-13** | `GET/POST /api/rider/scheduled-trips` (**body now takes `star?` 1\|2\|3, default 1, rejected 400 if out of range; `ScheduledTrip` returns `star`**), `GET/PUT /api/rider/scheduled-trips/{id}` (`star` not editable — cancel and rebook), `POST .../{id}/cancel`; **PRICE CHANGE: scheduled fares now price off the same per-star keys as immediate trips and have gone UP — a 20 km star-1 ride is 12,000 IQD, not 5,750 — see [Scheduled trips](#scheduled-trips-customer-app-require_role-rider)**; `GET/POST /api/rider/trips/{id}/stops` (max 3), **`GET /api/captain/trips/{trip_id}/stops`** (assigned-captain list — enumerate `stop_id`s), `POST /api/captain/trips/{trip_id}/stops/{stop_id}/reach` |
...
| Vehicle catalog (car picker) | **Live (2026-09-13)** | `GET /api/vehicle-catalog/brands` → `GET /api/vehicle-catalog/brands/{id}/models`, or `GET /api/vehicle-catalog/search?q=` for a bilingual type-ahead. **Public, no auth** (needed before a token exists). Send the chosen `car_brand_id` + `car_model_id` **and `car_year`** on `POST /api/captains/register` — this is what determines the car's star grade. Free-text `car_make`/`car_model` still works but grades poorly. See [Captain Car Star Class](#captain-car-star-class-live) |
| Fix my car details | **Live (2026-09-13)** | `PUT /api/captain/me/vehicle` `{car_make, car_model, car_color?, car_brand_id?, car_model_id?, car_year?}` → `200 Captain`. **A self-service edit can LOWER the star but never raise it** — a re-grade that would move the star up is queued for an operator to check against the registration document, and the 200 comes back with the old `star`. **A PUT, not a PATCH — `car_color`/`car_year` are cleared when omitted (omitting `car_year` drops the captain to star 1)**, so pre-fill the form from the current record. `409` (with `trip_id`) while on an `accepted`/`in_progress` trip; `400` on blank make/model, a year outside 1970..=2100, an unknown `car_brand_id`/`car_model_id`, or a model/brand mismatch. `car_plate` is not editable. See [Vehicle edit](#6-vehicle-edit--new-endpoints-2026-09-13) |
...
| Captains (approval queue + lifecycle + import) | 3 **Live** | `GET /api/captains` (excludes archived unless `?status=archived`), `GET /api/captains/pending`; `POST /api/captains/{id}/{approve,reject}` (admin), `.../{block,unblock,reconsider}` (super_admin); **`DELETE /api/captains/{id}` (super_admin) — soft-delete/archive**, **`DELETE /api/captains/{id}/purge` (super_admin) — permanent delete of an archived captain (204; trips detached, ledger kept)**; **`PUT /api/captains/{id}/star` (super_admin) — set car star grade 1/2/3, which also PINS it (`star_overridden_at`) against future re-grading; send `{"star": null}` to RELEASE the pin and re-grade from the stored car (see [Captain Car Star](#captain-car-star-class-live))**; **`PUT /api/captains/{id}/vehicle` (super_admin) — NEW 2026-09-13, restate a captain's car `{car_make, car_model, car_color?, car_brand_id?, car_model_id?, car_year?}` → `200 Captain` and re-grade it unless the star is pinned; the support-desk fix for a mis-entered car, and the one to reach for instead of `/star` when the *description* is what is wrong. Full replace for `car_color`/`car_year` (omit and they are cleared), but `car_brand_id`/`car_model_id` are always stored as whatever the grader RESOLVED — omit them and the free text is re-matched, it is not cleared. Unlike the captain-facing route this one re-grades in BOTH directions and clears any pending `star_review_requested_at`; `400` on blank make/model, year outside 1970..=2100, unknown `car_brand_id`, unknown `car_model_id`, or a model/brand mismatch; `404` unknown or archived; and NO mid-trip 409 on this route, unlike the captain-facing one (see [Vehicle edit](#6-vehicle-edit--new-endpoints-2026-09-13))**; **Abriyah access (admin): `POST /api/captains/{id}/approve` takes an optional `{ abriyah? }` scope; `POST /api/captains/{id}/abriyah/{approve,revoke}` grant/revoke later. The `abriyah/pending` queue and `abriyah/reject` are RETIRED — see [Captain Abriyah Access](#captain-abriyah-access-granted-at-approval-live)**; `POST /api/captains/bulk-import`; **`GET /api/admin/captains/{id}/history` → `ActivityHighlight[]`** (Captain Detail History tab — registration/approval/block events) |
...
| Box trips, fee and revenue | 15 **Live (2026-09-25)** | `trip_type` can now be **`"box"`** everywhere trip types appear (`GET /api/trips[/{id}]`, `GET /api/admin/operations/trips?type=box`, bulk export `trip_type`, search sublabels, trip-volume rows). **Box section:** `GET /api/trips/{id}/box` (any admin role) → `BoxDetails` (description, `photo_urls`, recipient name + phone, `surcharge_iqd`). **Box fee:** setting `pricing.box_surcharge_iqd` (seeded `"500"`, super_admin `PUT /api/admin/settings/pricing.box_surcharge_iqd` `{value}`, integer 0..=20000, else 400). **Reports:** revenue-by-zone rows gain `box_fare_iqd` (and CSV column); `total_fare_iqd` = regular + abriyah + box. See [Box delivery](#box-delivery-phase-15--live) |
| Peak times + distance pricing | 16 **Live (2026-09-25)** | **Rules:** `GET /api/admin/pricing/peak-rules` (any admin) → `{rules: PeakRule[]}`; `POST` (201) / `PUT .../peak-rules/{id}` (200, full replace) / `DELETE .../peak-rules/{id}` (204) -- **super_admin**; 400 `{"error":"invalid_peak_rule: <field>"}`; 404 unknown id. **Test a time:** `GET /api/admin/pricing/peak-rules/preview?at=<RFC3339>&trip_type=regular\|box[&lat=&lng=]` (any admin) → `{peak_enabled, max_percent, peak_percent, rule, matched_rule_ids}`. **Settings (existing settings PUT, super_admin):** `pricing.peak_enabled` (`"true"`\|`"false"`), `pricing.peak_max_percent` (0..200), `pricing.use_road_distance` (`"true"`\|`"false"`), `pricing.road_fallback_factor_percent` (100..250). **Trip detail:** `Trip.peak_percent` + `Trip.peak_extra_iqd` → "+20% (+750 IQD)". Audit feed: `pricing.peak_rule.{created,updated,deleted}`. See [Road-distance pricing & peak times](#road-distance-pricing--peak-times-phase-16--live) |
...
- **Fare estimate (public, no auth):** `GET /api/trips/estimate?pickup_lat=&pickup_lng=&dropoff_lat=&dropoff_lng=&trip_type=&zone_id=&promo_code=&star=` → `200 { fare_iqd, distance_km, base_fare_iqd, currency, discount_iqd, final_fare_iqd, surcharge_iqd, peak_percent, peak_extra_iqd, peak_label, distance_source }` (**`surcharge_iqd` added 2026-09-25**: the Box fee included in `fare_iqd` for `trip_type=box`, else 0 — see [Box delivery](#box-delivery-phase-15--live); **`peak_percent`/`peak_extra_iqd`/`peak_label`/`distance_source` and the optional `scheduled_for` param added 2026-09-25 (Phase 16)** — see [Road-distance pricing & peak times](#road-distance-pricing--peak-times-phase-16--live)). **Outside Iraq → `400 {"error":"outside_service_area"}` (2026-09-23).** **`star` (1|2|3, optional, default 1)** prices the normal-trip estimate for that car class (ignored when `trip_type=abriyah`) — see [Captain Car Star](#captain-car-star-class-live). **Fare model changed — now tiered (admin-configurable):** `fare = round_up_to(round_to_iqd, base + (min(km, threshold) × tier1_per_km) + (max(km − threshold, 0) × tier2_per_km))`. Defaults: base 2000 IQD, **the booked star's per-km rate — 500 (star 1) / 700 (star 2) / 1000 (star 3) IQD/km — charged on BOTH distance tiers, total rounded UP to the nearest 250 IQD**. The flat **250 IQD/km first-10-km, 125 IQD/km beyond** numbers this document used to quote were the retired `pricing.regular_tier{1,2}_per_km_iqd` keys; they have not priced a normal taxi trip since Car Star v2 (they still price an Abriyah room joined from a non-Abriyah pickup zone, which is why the keys are still in the settings table). `base_fare_iqd`, `pricing.tier_threshold_km` and `pricing.round_to_iqd` really are still shared across all three stars — only the per-km half is per-star, and because a star's two tier rates are currently equal, the 10 km threshold makes no difference to a normal fare today. `trip_type=abriyah` selects the Abriyah tiers/base (default base 0); any other value (or omitted) uses the normal-taxi tiers. **`zone_id` no longer changes the per-km rate** — pricing is now global per trip type (a zone's own per-km is ignored for fare math). **Distance is the ROAD distance (Phase 16)** from the self-hosted OSRM router; when routing is unavailable it is the straight line x `pricing.road_fallback_factor_percent` / 100 (default 140), and with `pricing.use_road_distance = "false"` the plain straight line (`distance_source` says which). During a peak-time rule `round(ride × peak_percent / 100)` is added before the round-up. All knobs are editable from admin settings — normal taxi: `pricing.regular_base_fare_iqd` plus the six per-star rates `pricing.star{1,2,3}_tier{1,2}_per_km_iqd`; Abriyah: `pricing.abriyah_base_fare_iqd`, `pricing.abriyah_tier1_per_km_iqd`, `pricing.abriyah_tier2_per_km_iqd`; shared: `pricing.tier_threshold_km`, `pricing.round_to_iqd` — and take effect on the next request (no redeploy). **Promo code (optional):** when `promo_code` is supplied and the code exists/active/within-window/under-global-cap, `discount_iqd` is the potential discount and `final_fare_iqd = max(0, fare_iqd - discount_iqd)`; an invalid/missing code returns `discount_iqd: 0` and `final_fare_iqd == fare_iqd` (never errors). This is a **potential** estimate only — the unauthenticated endpoint cannot check per-rider-once; `POST /api/trips` enforces all limits atomically.
- **Request (rider):** `POST /api/trips` `{ pickup_lat, pickup_lng, dropoff_lat, dropoff_lng, zone_id?, promo_code?, star?, pickup_address?, dropoff_address? }` → `201 Trip` (status `requested`, fare/distance computed, `base_fare_iqd` captured immutably). **Outside Iraq → `400 {"error":"outside_service_area"}` (2026-09-23).** **`pickup_address` / `dropoff_address` (NEW, LIVE 2026-08-14, optional strings ≤255 chars):** the human-readable place name the rider chose (e.g. a POI or search result like `"مول المنصور"`). Send them so the captain's offer card and the rider's post-booking screens show the chosen name instead of a reverse-geocoded label; omit to send coordinates only. Stored verbatim (trimmed; blank → null; silently capped at 255 chars). They are returned on the `Trip` object, on `GET /api/trips` / `GET /api/trips/{id}`, in **`GET /api/captain/trip-queue`** offers, and on the `rt:trip:{id}` `trip_update` and `rt:captain:{id}` offer frames. Null on trips created before this shipped (clients fall back to reverse-geocoding when null). Scheduled trips already carried address text and now thread it into the live trip on promotion. **Server-side auto-population (NEW, LIVE 2026-08-16, issue #10):** when the client omits an address (the single-map pin flow does), the backend best-effort fills it from the **nearest POI within 150 m** of the coordinate (Arabic-first name; null when no POI is that close). So `pickup_address`/`dropoff_address` may be non-null even when you sent only pins — the label is a nearby venue name, never a street/neighbourhood (the dataset is POI points only). Sending your own text still wins. **`star` (1|2|3, optional, default 1)** picks the car class — see [Captain Car Star](#captain-car-star-class-live). **Dispatch is no longer exact-only.** The offer goes out in two waves: first to captains graded **exactly** that star, then — if nobody has accepted `dispatch.star_widen_after_seconds` later (default **20 s**) and the trip is still `requested` — to captains graded **higher** than the booked star as well. **The rider always pays the tier they booked**, so widening can only hand them a *better* car than they paid for; there is never a fare surprise and no UI is needed for it. Out-of-range `star` is clamped to 1..3 on this endpoint (a scheduled trip rejects it with a 400 instead — see [Scheduled trips](#scheduled-trips-customer-app-require_role-rider)). **Promo code (optional):** if `promo_code` is supplied and valid, the trip is created with the **already-discounted `fare_iqd`** (net of the discount); `promo_code_id` and `discount_iqd` are set on the Trip object and the redemption is atomically reserved. If the code is invalid, expired, inactive, exhausted, or already used by this rider → **400** `{ "error": "..." }` and the trip is NOT created. Omitting `promo_code` behaves exactly as before. **A rider with an active (non-terminal) trip gets `409` — now carrying the blocking trip's id (issue #11):** `409 { "error": "rider already has an active trip", "active_trip_id": "<uuid>" }`. On that 409, read `active_trip_id` and deep-link the rider to the live-trip screen to cancel it, instead of dead-ending. Dispatch fans the offer out to eligible captains on `rt:captain:{id}` and publishes `beep.trip.requested` on `rt:trip:{id}`.
...
- **Accept (captain):** `POST /api/trips/{id}/accept` → `200 Trip` (status `accepted`, `captain_id`/`accepted_at` set). Works from `requested` (regular) **and** `matched` (Abriyah, Phase 6) — the handler routes through the state machine. **Three different things return `409` here, so branch on the body `error` string rather than on the status code:** `"captain star does not match the requested class"` (this captain is graded **below** the trip's `requested_star` — not retryable, drop the offer), `"captain already has an active trip"` (finish or cancel the current ride first), and anything else (someone else took it, or it was cancelled — refresh the queue). **Equal or higher star is accepted**, which is what makes the widen wave usable: a star-2 captain may take a widened star-1 trip, and the rider still pays star 1. Abriyah (`matched`) trips ignore the star grade entirely.
...
- **Trip object** key fields: `id, trip_type ("regular"|"abriyah"|"box"), status, rider_id, captain_id?, zone_id?, room_id?, pickup_lat/lng, dropoff_lat/lng, fare_iqd, distance_km, base_fare_iqd, currency, fare_per_rider_iqd?, distance_per_rider_km?, promo_code_id?, discount_iqd?, cancellation_reason?, cancelled_by?, requested_at, accepted_at?, started_at?, completed_at?, cancelled_at?, version, peak_percent, peak_extra_iqd` (**`peak_percent`/`peak_extra_iqd` added 2026-09-25, Phase 16**: the peak surcharge snapshotted when the fare was struck, 0 when none; `peak_extra_iqd` is already inside `fare_iqd`). `distance_km` is the km actually priced (road km, or the fallback). When a promo was applied, `fare_iqd` is already net of the discount; `discount_iqd` is the amount deducted; `promo_code_id` is the UUID of the applied code.
...
A **1/2/3-star grade on each captain's car** (3 = nicest). Riders pick a star when requesting a **normal** trip and pay that star's per-km fare. **Abriyah ignores star entirely** (no star field on rooms; Abriyah keeps its own pricing and pooling).

### REWRITTEN 2026-09-13 — read this if you integrated the old behaviour

The previous version graded on **model year alone** against absolute cutoffs, and it never actually worked in production: `car_year` was `null` for **every** captain, so all of them sat at star 1, and **every star-2 and star-3 ride request ever made was cancelled without a match** (12 requested, 12 cancelled, 0 completed). Five things changed:

1. **Grading now uses a real vehicle catalog**, not just the year.
2. **Dispatch widens upward** instead of stranding star-2/3 riders.
3. **The three tiers now cost different amounts.** They were all 500 IQD/km before.
4. **Brand ordering in the catalog is fixed** — the picker no longer opens on the rarest makes in Iraq (section 1).
5. **A car can finally be corrected after registration**, by the captain or by support, without pinning the grade (section 6). The "no vehicle-edit endpoint" gap this section used to list as unshipped is closed.

### 1. Vehicle catalog — NEW endpoints (public, no auth)

A canonical brand/model catalog (**141 brands, 2,414 models**, English + Arabic names) now backs car entry. **Captain App: replace the free-text car make/model inputs with a picker.**

| Endpoint | Returns |
|-|-|
| `GET /api/vehicle-catalog/brands` | All brands: `{ id, name_en, name_ar, is_premium }`. Ordered for display (common-in-Iraq makes first, then A→Z) — **this ordering was broken on first release and is fixed as of 2026-09-13, see below**. |
| `GET /api/vehicle-catalog/brands/{id}/models` | That brand's models: `{ id, brand_id, name_en, name_ar }`. Unknown brand → `200` with `[]`, not `404`. |
| `GET /api/vehicle-catalog/search?q=` | Type-ahead across all brands+models, **both languages** — `corol`, `Corolla`, `كورولا`, `تويوتا` all work. Returns `{ id, brand_id, name_en, name_ar, brand_name_en, brand_name_ar }`, best match first, max 25. `q` shorter than 2 chars → `[]`. |

**Brand ordering is FIXED (2026-09-13).** `GET /api/vehicle-catalog/brands` now genuinely returns the common-in-Iraq makes first. The list shipped inverted: `sort_order` is a **rank** (1 = the make most common on Iraqi roads, 141 = the rarest) and the query sorted it descending, so the picker opened on `LYNK & CO`, `JAECOO`, `OMODA`, `XPeng`, `Oldsmobile` — five makes almost nobody in Baghdad drives — and Toyota was 141st, past the end of the first screen. The list now opens `Toyota, Soueast, Mercedes-Benz, Kia, BYD, GAC, Nissan, Jetour, Mitsubishi, Volkswagen, Geely, Hyundai, …`, ties broken A→Z by `name_en`, and any brand added later without a rank sorts **after** every ranked one instead of jumping to the top. All 141 brands carry a real rank today. **Captain App: the workaround can be dropped.** If you pinned a popular-makes list by name at the top of the picker, it is not wrong — it will keep working — but it is now duplicating what the server does, and it will not pick up new makes the catalog ranks later. Render the response in the order it arrives.

**Registration body gains two optional fields** on `POST /api/captains/register`:

```jsonc
{
  "car_brand_id": "uuid",   // NEW - from the catalog. Preferred.
  "car_model_id": "uuid",   // NEW - from the catalog. Preferred.
  "car_year": 2019,         // strongly recommended (see below)
  "car_make": "Toyota",     // still accepted, still stored, now only a fallback
  "car_model": "Corolla"    // still accepted, still stored, now only a fallback
}
```

**Nothing breaks if you send neither.** Free text is fuzzy-matched (Arabic and English) to the catalog as a bridge for shipped builds. But the match is deliberately strict, and the values live captains actually typed — `كوري`, `كوريا`, `امريكا`, `الصين` (countries of origin), `2026`/`2023` in the *model* field, `258`, `طراز` (the literal Arabic word "model") — **match nothing**, so those captains are graded on year alone. Sending the ids is how a captain gets an accurate grade.

### 2. How the grade is computed

Two independent gates, both must pass. This mirrors how Careem and Bolt tier cars in the region.

- **Hard age cap** (relative, so it no longer needs rolling forward every year): star 3 requires the car be **≤ 5 model years old**, star 2 **≤ 10**. Outside the cap → star 1 no matter how good the car is. A 2004 S-Class is star 1.
- **Quality score 0–100** built from the catalog: year-adjusted market value from ~4,800 real Iraqi listings, rear-passenger comfort (rear A/C vents weighted heavily — the rider is in the back and Baghdad summers run near 50 °C), airbag count, recency, and a premium-marque bonus. Star 3 needs ≥ 70, star 2 needs ≥ 45.

Consequences worth knowing:

- **`car_year` is now the single most valuable field you can collect.** With no year, a car clears no age gate and grades **star 1**, regardless of what it is.
- **An unidentified car can never be star 3.** If the model doesn't resolve to the catalog, the grade is capped at star 2 even for a brand-new car — otherwise anyone typing nonsense plus a recent year would be handed the premium tier.
- **Same brand, different grade:** a current Toyota Corolla is star 2; a current Toyota Land Cruiser is star 3. This is the whole point of the change.
- Settings are admin-tunable and take effect on the next registration: `classifier.star2_max_age_years` (10), `classifier.star3_max_age_years` (5), `classifier.star2_min_score` (45), `classifier.star3_min_score` (70). The old `classifier.star{2,3}_min_year` keys are **no longer read**.

### 3. Dispatch now widens upward (fixes the 100% cancellation)

**Matching is no longer exact-only.** A star-N request is offered first to captains graded **exactly** N. If nobody has accepted **20 seconds** later and the trip is still `requested`, it is offered to captains graded **higher** than N as well.

- **The rider is always charged the tier they booked.** Widening can only hand them a *better* car than they paid for — there is never a fare surprise, and no UI is needed for it.
- The two waves are disjoint, so a captain never receives the same offer twice.
- Tunable via `dispatch.star_widen_after_seconds` (default `20`; set very high to restore the old exact-match behaviour).
- **Customer App:** you can stop worrying that picking star 2 or 3 means never getting a car. The previous "picks a class with no online captains simply waits" caveat is gone.

### 4. Per-star pricing is now actually different

| Tier | Per-km (both distance tiers) | 10 km trip | 20 km trip |
|-|-|-|-|
| star 1 | **500 IQD/km** (unchanged) | 7,000 IQD | 12,000 IQD |
| star 2 | **700 IQD/km** (1.4×) | 9,000 IQD | 16,000 IQD |
| star 3 | **1,000 IQD/km** (2.0×) | 12,000 IQD | 22,000 IQD |

(Worked examples include the shared 2,000 IQD base fare and the round-up to 250 IQD: `2000 + km × rate`. **Corrected 2026-09-13** — the 10 km column previously read 6,000 / 8,000 / 11,000, which silently assumed a 1,000 IQD base; the seeded `pricing.regular_base_fare_iqd` is **2000**, as the fare-model bullet in [Regular Trips](#regular-trips-phase-5--live) has always said. Quote fares from `GET /api/trips/estimate`, never from this table.)

`GET /api/trips/estimate?...&star=N` now returns **three different fares** for the three stars — previously all three came back identical. **Customer App: if you cached or hardcoded around the tiers being equal, undo that**, and show the real per-tier price in the class picker. Base fare, distance threshold, and rounding are still shared (`pricing.regular_base_fare_iqd`, `pricing.tier_threshold_km`, `pricing.round_to_iqd`). Rates remain admin-editable at `pricing.star{1,2,3}_tier{1,2}_per_km_iqd`.

### 5. Admin

- **Override / re-grade (super_admin):** `PUT /api/captains/{id}/star` `{ star: 1|2|3 }` → `200 Captain`. Unchanged contract, but it now also **pins** the grade — an admin-set star is recorded with `star_overridden_at` and is skipped by every re-grade thereafter, so a human decision is never silently undone. `star` outside 1..3 → `400`; unknown captain → `404`; non-super_admin → `403`. Publishes `beep.captain.star_changed`.
- **Release the pin (super_admin) — NEW:** the SAME route with `{ "star": null }` → `200 Captain`. It clears `star_overridden_at` **and immediately re-grades the captain from the car on file**, in one call, so the row never sits unpinned at a stale hand-set number. This is the **only** way out of a pin: sending a number re-pins, and a vehicle edit deliberately refuses to move a pinned star, so without this the first hand-set grade would freeze that captain's per-km rate for the life of the account. Use it once the reason for the override is gone (the car was corrected, or the classifier was fixed). Either form also clears any pending `star_review_requested_at`.
- **Captain object — THREE NEW FIELDS (2026-09-13).** On top of `star` (1..3) and `car_year` (i16 or null), the `Captain` payload now also carries:

| Field | Type | Meaning |
|-|-|-|
| `car_brand_id` | `uuid \| null` | The catalog brand the car resolved to. Null when it never resolved — either the captain registered on a pre-picker build and their free-text `car_make` fuzzy-matched nothing, or the make is not in the catalog. |
| `car_model_id` | `uuid \| null` | The catalog model the car resolved to. Null for the same reasons, **and this is the field that explains a stuck star 2**: with no model there are no quality signals to grade on, so grading falls back to model year alone and is capped at star 2. Filtering a captain list on this being null finds exactly the cars that can never reach star 3 until someone fixes them. |
| `star_overridden_at` | `RFC3339 \| null` | When an admin last set `star` by hand. **Non-NULL means the grade is PINNED**: a human decided it, so it survives a vehicle edit untouched. Null means the grade is whatever the classifier last produced. Clear it with `PUT /api/captains/{id}/star` `{"star": null}`. |
| `star_review_requested_at` | `RFC3339 \| null` | **NEW.** When this captain's own `PUT /api/captain/me/vehicle` asked for a grade INCREASE that was withheld pending review. Non-NULL = a work item: read the captain's `car_make`/`car_model`/`car_brand_id`/`car_model_id`/`car_year`, compare against their `car_registration` document, and either apply the raise with `PUT /api/captains/{id}/vehicle` or pin the grade with `PUT /api/captains/{id}/star` — **both clear the flag**, as does any later vehicle edit (the claim has changed). Filter the captain list on it being non-null to get the queue. |

**Where they appear:** everywhere a `Captain` is returned: `POST /api/captains/register`, `GET /api/captains/{id}`, and the approve/reject/block/star/vehicle responses. **`GET /api/captains` list rows are different** — `CaptainRow` carries `car_brand_id`, `car_model_id`, `star_overridden_at` and `star_review_requested_at` but **not** `star` or `car_year`; fetch the captain by id when you need the grade itself.
- **Pinning is a standing decision, so prefer fixing the car.** `PUT /api/captains/{id}/star` stamps `star_overridden_at` and exempts that captain from re-grading until someone releases it with `{"star": null}`. Use the vehicle-edit endpoints below to correct a *wrong description*; reserve `/star` for a grade you genuinely want frozen against the classifier.

### 6. Vehicle edit — NEW endpoints (2026-09-13)

The previous version of this document said there was **no vehicle-edit endpoint** and that a mistyped car could only be papered over with `PUT /api/captains/{id}/star`. That gap is closed. Two routes now restate a captain's car and re-grade it; they share one service, so the validation, the re-grade and the pin rule are identical on both.

| Route | Who | Guard |
|-|-|-|
| `PUT /api/captain/me/vehicle` | The captain, on their own record | Captain token; the record is taken from the token `sub`, so a captain can never edit anyone else's car |
| `PUT /api/captains/{id}/vehicle` | Support / ops | **super_admin only** — the same guard as `PUT /api/captains/{id}/star`, because both rewrite what a captain earns per km |

**Body (identical on both routes)** — the field names and meanings are exactly the vehicle half of `POST /api/captains/register`, so the Captain App can reuse its registration form:

```jsonc
{
  "car_make":     "Toyota",   // REQUIRED, non-blank
  "car_model":    "Corolla",  // REQUIRED, non-blank
  "car_color":    "white",    // optional
  "car_brand_id": "uuid",     // optional, from the catalog
  "car_model_id": "uuid",     // optional, from the catalog
  "car_year":     2019        // optional, 1970..=2100 — but see the trap below
}
```

**This is a PUT, not a PATCH — but only for two of the four optional fields.** `car_color` and `car_year` are **CLEARED** when you omit them, and the trap that matters is that **omitting `car_year` drops the captain to star 1**, because a car with no year clears no age gate.

`car_brand_id` and `car_model_id` do **not** work that way and omitting them does **not** clear them. Whatever you send, the server stores the ids the **grader resolved**: from the ids you sent, or — when you send none — from fuzzy-matching the free-text `car_make`/`car_model` against the catalog. So a captain who was wrongly fuzzy-matched to the wrong model and then re-sends the same free text hoping to reset it gets **the same wrong model back**, populated in the 200 body. The only way to change the catalog identification is to send the ids you want. Either way: always send the full block, pre-filled from the captain's current record, and read the ids back off the response.

**`car_plate` is deliberately not editable here.** The plate is the car's legal identity, is unique across captains, and is on the rider's screen and the trip record; changing it is a heavier operation than fixing a mistyped year.

**The re-grade rule, stated plainly.** The star is recomputed through the **same classifier registration uses**, and then two things can hold the new grade back:

1. **A pin.** If `star_overridden_at` is set, `star` is left exactly as the admin decided and only the description changes — on *both* routes. A captain editing their own car can therefore never silently undo a deliberate admin override. The catalog ids **are** still resolved and stored for a pinned captain (what was pinned is the *grade*, not the claim about which car this is), so the grade is correct the moment the pin is released — with `PUT /api/captains/{id}/star` `{"star": null}`, which is the one call that clears it (sending a *number* to that route re-pins).
2. **Direction, on the captain's own route only.** `PUT /api/captain/me/vehicle` can **lower** a grade immediately but cannot **raise** one. A re-grade that would move the star up stores the description and the catalog ids, leaves `star` where it was, and stamps `star_review_requested_at` for an operator to check against the `car_registration` document. This is not a bug to work around: the body is an unverified self-declaration, the catalog ids come from unauthenticated endpoints, `car_plate` is not editable but does not constrain make/model, and the difference between star 1 and star 3 is 500 vs 1,000 IQD per km.

**Captain App:** after a 200, read `star` back off the body. If it did not move to what you expected, the raise is **pending review, not lost** — say so ("your car details were updated; the new class is being reviewed") rather than showing an error or silently re-quoting the old grade. `PUT /api/captains/{id}/vehicle` (super_admin) re-grades in **both** directions and clears the flag: the operator *is* the review.

**Status codes:**

| Code | When |
|-|-|
| `200` | Updated. Re-graded unless pinned, or (captain route only) unless the re-grade would raise the star, in which case the raise is queued. Body is the full `Captain`, so read `star`, `car_brand_id`, `car_model_id`, `star_overridden_at` and `star_review_requested_at` straight back off the response instead of re-fetching. |
| `400` | Blank `car_make`/`car_model`; `car_year` outside 1970..=2100; a `car_brand_id` that is not in the catalog; a `car_model_id` that is not in the catalog; or a `car_model_id` that does not belong to the `car_brand_id` you sent. **Both ids are validated independently** — a brand id sent with no model id is checked too (it used to fall through to the FK and surface as a `500`). A mismatched pair is **rejected, not silently stored** — storing it would grade the captain as a car they did not pick. |
| `401` | Missing or invalid bearer token |
| `403` | Wrong role (not a captain on `/captain/me/vehicle`; not a super_admin on `/captains/{id}/vehicle`) |
| `404` | No live captain for the token subject / for `{id}` — including an archived (self-deleted) captain, which must read as no account |
| `409` | **`PUT /api/captain/me/vehicle` only** — the captain is on an `accepted` or `in_progress` trip. Body: `{ "error": "finish the active trip before changing your vehicle", "trip_id": "<uuid>" }`. Read `trip_id` and deep-link to the live trip, exactly as with the `POST /api/trips` 409. |

**The admin route is deliberately NOT 409-guarded mid-trip.** A support ticket arrives precisely *while* the captain is working; an operator who cannot touch the record until the captain goes idle has no lever at all, and the operator has the call context the captain lacks. The captain-side guard stays because the rider is looking at a make, model and colour on their screen, and a captain's own edit has no reason that cannot wait until the ride ends.

**Send `car_model_id`.** A car that matches no catalog model is graded on model year alone and is **capped at star 2** — a self-service edit that only fixes the free text cannot lift a captain to star 3.

**Audit:** both routes publish `beep.captain.vehicle_changed` on `rt:admin:ops` with `{ id, by, car_make, car_model, car_year, car_brand_id, car_model_id, star_before, star_after, star_pinned }`. `star_pinned: true` is how a reader tells "the grade was held because an admin pinned it" from "the re-grade happened to land on the same tier".
...
- **Settings (Setup):** `GET /api/admin/settings` → `{ settings: Setting[] }` (**35 keys as of migration 058**: `pricing.*` — including the six per-star per-km rates — `classifier.*` (the star grading thresholds), `dispatch.star_widen_after_seconds`, `activation.daily_fee_iqd`, `trip.cancellation_penalty_iqd`, `room.default_max_*`, `room.allow_women_only_globally`, `general.*`). Read the list from the endpoint rather than hardcoding it; the count grows with each pricing or grading change. `GET /api/admin/settings/{key}` → `Setting { key, value (string), updated_by?, updated_at }`. `PUT /api/admin/settings/{key}` `{ value }` (super_admin) → updated `Setting`. **Range-validated** per key (e.g. `room.default_max_riders` 2-6, `activation.daily_fee_iqd` 500-10000, per-km 100-10000) — out of range → 400. **Settings are non-retroactive**: changing a value does NOT rewrite existing zones/activations (but fare keys ARE live for NEW estimates/bookings immediately — fares are computed per request). The `activation.daily_fee_iqd` setting now drives the captain Activate-Today fee (was a 2000 constant); zone-creation defaults read `pricing.*`/`room.*`.
  - **Tiered fare pricing keys (drive the live fare engine for both apps):** normal taxi → `pricing.regular_base_fare_iqd` (base, 500–10000, default 2000) plus **six per-star per-km keys, `pricing.star{1,2,3}_tier{1,2}_per_km_iqd`** (100–10000; defaults **500 / 700 / 1000 IQD/km**, the same rate on both distance tiers for a given star); Abriyah → `pricing.abriyah_base_fare_iqd` (base, 0–5000, default 0), `pricing.abriyah_tier1_per_km_iqd` (100–10000, default 250), `pricing.abriyah_tier2_per_km_iqd` (100–10000, default 125); shared → `pricing.tier_threshold_km` (km where tier2 begins, 1–100, default 10) and `pricing.round_to_iqd` (round the final fare UP to this multiple, 1–1000, default 250). **Corrected 2026-09-13:** this bullet used to say a normal taxi runs at 250 IQD/km then 125 IQD/km — those are the `pricing.regular_tier1_per_km_iqd` / `pricing.regular_tier2_per_km_iqd` keys, and **editing them no longer changes a normal taxi fare**; the per-star keys do. The `regular_tier*` keys survive because an Abriyah room joined from a **non-Abriyah** pickup zone is still priced from them. The legacy flat `pricing.regular_per_km_iqd` / `pricing.abriyah_per_km_iqd` keys remain in the table (used only for zone-creation defaults) but **no longer affect fare computation**. **Scheduled trips read the per-star keys too** (changed 2026-09-13; see [Scheduled trips](#scheduled-trips-customer-app-require_role-rider)) — they used to read `regular_tier*`, so a rate edit that an operator expects to move every normal fare now has to be made on the star keys or it moves nothing.
...
- **Row shapes (key metric columns):**
  - **Trip Volume:** `zone_name, trip_type (regular|abriyah|box), room_type (n/a|mixed|women_only), total_trips, completed, cancelled, total_fare_iqd, avg_fare_iqd`.
  ...
  - **Revenue by Zone:** `zone_name, trip_count, total_fare_iqd, regular_fare_iqd, abriyah_fare_iqd, box_fare_iqd` (**`box_fare_iqd` added 2026-09-25**; `total_fare_iqd` = regular + abriyah + box).
...
> **PRICE CHANGE, LIVE 2026-09-13 — scheduled fares went UP.**
>
> Scheduled trips used to be priced from the retired flat `pricing.regular_tier{1,2}_per_km_iqd` keys (250 IQD/km, tapering to 125 IQD/km past 10 km) while an immediate `POST /api/trips` priced from the per-star keys (500/700/1000 IQD/km, no taper). Booking ahead was therefore **cheaper than booking now for the same route**, and the further the ride the wider the gap — nobody decided that; it fell out of which settings key each code path happened to read. Both now read the **same per-star keys**.
>
> **EVERY scheduled fare rises, not just the long ones.** The legacy rate (250 IQD/km) was never equal to star 1 (500 IQD/km) at any distance, so there is no "short rides are unaffected" band: every km under the 10 km threshold doubles (250 → 500 IQD/km) and every km past it quadruples (125 → 500 IQD/km). At the seeded rates a 5 km ride goes **3,250 → 4,500** (+38%), a 10 km ride **4,500 → 7,000** (+56%), and a 20 km ride **5,750 → 12,000** (+109%) — which is exactly what that 20 km ride has always cost booked immediately. Star 2 and star 3 are higher again (700 and 1,000 IQD/km).
>
> **Customer App:** if any screen quotes a scheduled fare from a cached number, a hardcoded rate, or a pre-booking estimate taken before this shipped, it is now under-quoting — by roughly a third on a short ride, by half on a 20 km one, and by more the longer the ride. Quote scheduled trips from `GET /api/trips/estimate` with the same `star`, and say plainly that the fare is struck at promotion from the rates in force then — the booking itself never returns a fare.

- `POST /api/rider/scheduled-trips` `{ trip_type:"regular", pickup_lat/lng, pickup_address?, dropoff_lat/lng, dropoff_address?, scheduled_for, star? }` → 201 `ScheduledTrip` (status `pending`). **`scheduled_for` must be 30 min – 7 days out** (else 400); **regular only** (abriyah → 400).
- **`star` (1|2|3, optional, default 1) — NEW 2026-09-13.** Books a car class ahead of time; before this, promotion hardcoded star 1, so a rider simply could not ask for a star 2 or 3 car in advance — dispatch only ever looked for star-1 captains. The booked `star` both **prices** the trip (at that star's per-km rate) and is carried onto the promoted trip's `requested_star`, so the class that was paid for is the class dispatch searches, two-wave widening included. **An out-of-range `star` is rejected with `400`, not clamped** — deliberately unlike `POST /api/trips`, which clamps: an immediate trip quotes its fare back in the 201 a second later, while a scheduled fare is struck at promotion hours later with nobody watching, so a silently clamped class would surface only as a surprise charge. `ScheduledTrip` responses carry `star`.
- `GET /api/rider/scheduled-trips` → the rider's own list. `GET /api/rider/scheduled-trips/{id}` (owner only). `PUT .../{id}` updates time/pickup/dropoff (pending only; same time-window guard) — **`star` is NOT editable there**: the class sets the price, so changing it means cancel and rebook. `POST .../{id}/cancel` `{ reason? }` → `cancelled` (owner + pending only, else 403/409).
- A background scheduler tick (every 60s) **promotes** a due pending trip into a live REQUESTED trip (`status → promoted`, `promoted_trip_id` set) and hands it to the **same dispatcher `POST /api/trips` uses** — ranked nearest-captain queue, `rt:captain:{id}` offers, `new_trip_in_queue` FCM pushes and the two-wave star widening, all identical to an immediate booking. (Until 2026-09-13 promotion only published `beep.trip.requested` and stopped there, so a promoted trip reached no captain's phone and was findable only by whoever happened to be polling `GET /api/captain/trip-queue`.) From the rider's side a promoted trip is an ordinary `requested` trip: poll `GET /api/trips/{id}` or subscribe to `rt:trip:{id}`. Overdue-by-5-min pending trips are marked `expired`. `ScheduledTrip.status`: `pending | promoted | cancelled | expired`.
...
## Box delivery (Phase 15 — Live)

A rider sends items with a captain. A Box trip is a normal point-to-point trip with `trip_type: "box"`: same lifecycle (`requested → accepted → in_progress → completed | cancelled`), same star classes and dispatch waves, same accept / arrive / start / complete / cancel / rate / proxy-call endpoints. What is new is the price (a small Box fee on top of the star fare) and a **parcel** (description, 0-3 photos, recipient) that only the trip's participants can read. **Not supported:** scheduled Box, Box in Nafarat rooms, multi-stop Box, SMS to the recipient.

**1. Photos (optional, 0-3) — `POST /api/rider/box/photos/upload-url`** (rider token only; any other role → 403; no body) → `200 PresignedUploadResponse`:
```json
{ "upload_url": "https://…presigned PUT…", "object_key": "box-photos/<rider_id>/<uuid>", "expires_in": 300 }
```
Call it **once per photo** (every call mints a new key), then `PUT` the raw image bytes to `upload_url` with header `Content-Type: image/jpeg` and **no** `Authorization` header — the same flow as the rider avatar. Keep the `object_key`s in display order. With mock storage (local dev) the URLs are `https://mock-storage.local/...` placeholders.
- **Enforced (2026-09-25 review):** `Content-Type: image/jpeg` is a **signed header** of the Box upload URL — a PUT with any other content type is refused by storage (403). Send JPEG only (compress/convert before upload). The avatar and captain-document URLs are unchanged.
- **Checked at booking:** each `photo_keys` entry must have actually been uploaded, be an image, and be **at most 8 MiB**; otherwise `POST /api/trips/box` answers `400 invalid_box_photos`. Only book once every photo PUT has returned 2xx.

**2. Price — `GET /api/trips/estimate?trip_type=box&star=1|2|3&pickup_lat=&pickup_lng=&dropoff_lat=&dropoff_lng=[&promo_code=]`** (public) → `FareEstimateResponse` with:

| Field | Box value |
|-|-|
| `fare_iqd` | the star's regular fare **plus the Box fee, added before the round-up** (so it stays on the 250 IQD grid) |
| `surcharge_iqd` | the Box fee included in `fare_iqd` (current `pricing.box_surcharge_iqd`, default 500); **0 for regular and abriyah** |
| `discount_iqd` / `final_fare_iqd` | potential promo discount on the Box fare / `max(0, fare_iqd - discount_iqd)` |

`trip_type=box` respects `star`; unknown `trip_type` values still price as regular. Since Phase 16 the distance is the road distance and `fare_iqd` also includes any peak surcharge (on the ride part only, never the Box fee) -- see [Road-distance pricing & peak times](#road-distance-pricing--peak-times-phase-16--live). Clients never compute fares — show `fare_iqd` and "includes Box fee `surcharge_iqd`".

**3. Book — `POST /api/trips/box`** (rider token only → else 403; rate-limited like `POST /api/trips`). Body `CreateBoxTripDto`:
```json
{
  "pickup_lat": 33.31, "pickup_lng": 44.36, "dropoff_lat": 33.30, "dropoff_lng": 44.40,
  "pickup_address": "optional string", "dropoff_address": "optional string",
  "zone_id": "optional uuid", "star": 1, "promo_code": "optional string",
  "description": "Box of clothes and two books",
  "recipient_phone": "07701234567",
  "recipient_name": "optional string",
  "photo_keys": ["box-photos/<rider_id>/<uuid>"]
}
```
Validation runs in this order; each failure is `400 {"error":"<code>"}` with the bare code:

| Check | `error` |
|-|-|
| pickup or dropoff outside Iraq | `outside_service_area` |
| trimmed `description` not 3..=500 characters, missing/null, or containing a control character other than line break / tab | `invalid_box_description` |
| `recipient_phone` missing/null or not normalizable to `9647XXXXXXXXX` (accepted: `07XXXXXXXXX`, `7XXXXXXXXX`, `9647XXXXXXXXX`, `+9647XXXXXXXXX`, `009647XXXXXXXXX`; Arabic-Indic digits and spaces tolerated) | `invalid_recipient_phone` |
| more than 3 `photo_keys`, a duplicate, a key not exactly `box-photos/{your rider id}/{lowercase uuid}` as issued, or a photo that is not in storage, not an image, or over 8 MiB | `invalid_box_photos` |
| invalid promo code | the existing promo 400s, unchanged |

Then exactly like `POST /api/trips`: `409 {"error":"rider already has an active trip","active_trip_id":"…"}` when one exists; `star` clamped 1..3 (default 1); addresses trimmed and auto-filled from the nearest place; promo reserved on the Box gross fare. `recipient_name` has control characters removed, is trimmed (blank → null, max 120 chars); `photo_keys` may be omitted, `null` or `[]`. **Response `201 Trip`** (the existing schema) with `trip_type: "box"`, `status: "requested"` and `fare_iqd` including the Box fee (net of any promo). The trip and its parcel are stored in one transaction. Dispatch starts immediately (same waves as regular).

**4. Parcel details — `GET /api/trips/{id}/box`** — the **only** place the recipient phone and photos are served. `200 BoxDetails`:
```json
{
  "trip_id": "uuid",
  "description": "Box of clothes and two books",
  ...
  "recipient_name": "Ali",
  "photo_urls": ["https://…presigned GET (~5 min)…"],
  "surcharge_iqd": 500
}
```
`recipient_name` may be `null`; `photo_urls` keeps the order of the booked `photo_keys` (empty when none). URLs expire in about 5 minutes — re-fetch for fresh ones (use a stable image cache key, e.g. trip id + index). Access:

| Caller | Allowed when |
|-|-|
| rider | owns the trip (any status) |
| captain | is the trip's assigned captain **and** status is `accepted` or `in_progress` **and** the captain account is `approved` (a blocked captain loses access at once) |
| `super_admin` / `operator` / `finance` | always |

Anyone else → `403 {"error":"forbidden"}`. Missing trip, or a trip that is not `box` → `404`.

**5. Captain side.** Box trips appear in `GET /api/captain/trip-queue` like regular ones (same star window). New `CaptainOffer` fields (additive):

| Field | Type | Value |
|-|-|-|
| `trip_type` | string \| null | `"regular"` or `"box"` for trip offers; `null` for room offers |
| `box_description` | string \| null | the parcel description (full, max 500); `null` for non-box |
| `box_photo_url` | string \| null | presigned GET of the **first** photo, or `null` |
| `box_photo_count` | int | number of photos (0 for non-box) |

The WS offer frame on `rt:captain:{id}` gains `"trip_type"` (offers are still recognised by `trip_id` + `pickup_lat`). Box FCM offers keep `notification_type: "new_trip_in_queue"` with title "New delivery nearby" / body "A customer wants to send a box near you.". Accept with `POST /api/trips/{id}/accept` (a captain graded below `requested_star` gets the same 409). **The recipient phone is never in an offer, WS frame, push or the audit log**; after accepting, read it from `GET /api/trips/{id}/box`. Calling the **sender** still uses the masked proxy (`GET /api/captain/trips/{id}/proxy`).

**6. Pushes to the rider** (types unchanged): `trip_accepted` → body "A captain is coming to pick up your box"; `trip_completed` → title "Box delivered", body "Your box has been delivered".

**7. Admin.** `trip_type` filter value `box` works on `GET /api/admin/operations/trips?type=box` and the bulk export. Trip detail Box section = `GET /api/trips/{id}/box`. Box fee setting `pricing.box_surcharge_iqd` (super_admin; integer 0..=20000; seeded `"500"`; takes effect on the next estimate/booking). Revenue-by-zone gains `box_fare_iqd`; trip-volume rows get `trip_type: "box"`.

## Road-distance pricing & peak times (Phase 16 — Live)

Two pricing changes, both server-side. **Clients never compute fares** -- show `fare_iqd` / `final_fare_iqd` as before.
...
| Rule | Days | Time | Surcharge |
|-|-|-|-|
| Morning rush | Sun-Thu `[0,1,2,3,4]` | 07:30-09:00 | +20% |
| Afternoon rush | Sun-Thu `[0,1,2,3,4]` | 14:00-15:30 | +20% |
| Evening crowd | every day `[0..6]` | 19:30-21:30 | +10% (citywide; restrict with `zone_ids`) |

Fare math: `ride = base + tiered(km)`; `fare_iqd = round_up_to(round_to_iqd, ride + round(ride x peak_percent / 100) + box_fee)`; `peak_extra_iqd = fare_iqd - round_up_to(round_to_iqd, ride + box_fee)` (exactly what peak added after rounding; can be 0 when rounding absorbs a small percent). Promo codes discount the full `fare_iqd`. A **scheduled** trip is charged the peak in force at its **scheduled pickup time**, not when it was booked.

**Estimate — `GET /api/trips/estimate`** (public) gains (additive):

| Field | Type | Meaning |
|-|-|-|
| `peak_percent` | int | percent in force; `0` when none (always 0 for Abriyah) |
| `peak_extra_iqd` | int | IQD peak adds, **already included in `fare_iqd`**; `0` when none |
| `peak_label` | string \| null | the winning rule's `name` (admin-entered, may be Arabic); `null` when `peak_percent` is 0 |
| `distance_source` | string | `"road"` \| `"straight_fallback"` \| `"straight"` |

New optional query param `scheduled_for` (RFC3339, e.g. `2026-09-27T07:45:00%2B03:00` -- URL-encode the `+`): evaluate peak at that pickup time instead of now; unparseable → `400 {"error":"invalid_scheduled_for"}`.

**Trip** (every endpoint returning `Trip`) gains `peak_percent: int` and `peak_extra_iqd: int` -- the snapshot taken when the fare was struck (0 on older trips and Abriyah). `fare_iqd` already includes `peak_extra_iqd`.

**Per client:**
- **Customer App: no change (product decision).** The rider app shows nothing about peak time. Ignore the new fields; the committed receipt computes the distance line as `total - base - box fee`, so a peak amount folds into it and the breakdown still sums to `fare_iqd`.
- **Captain App: no change.** Offer and trip fares (`fare_iqd`) already include everything.
- **Admin Dashboard:** the Peak times setup page, the Distance card and a "Peak" field on trip detail, using the endpoints below.

**Admin endpoints** (tag `Pricing`, admin JWT):

| Method | Path | Role | Response |
|-|-|-|-|
| GET | `/api/admin/pricing/peak-rules` | any admin | `200 {"rules": PeakRule[]}`, ordered by `start_time`, then `name` |
| POST | `/api/admin/pricing/peak-rules` | super_admin | body `PeakRuleInput` → `201 PeakRule` |
| PUT | `/api/admin/pricing/peak-rules/{id}` | super_admin | body `PeakRuleInput` (full replace; the enabled toggle sends the whole rule) → `200 PeakRule`; `404` unknown id |
| DELETE | `/api/admin/pricing/peak-rules/{id}` | super_admin | `204`; `404` unknown id |
| GET | `/api/admin/pricing/peak-rules/preview?at=<RFC3339>&trip_type=regular\|box[&lat=<f64>&lng=<f64>]` | any admin | `200 PeakPreview` |

`PeakRule`:
```json
{ "id": "0a640001-5c7b-4e2a-9d31-6b0c1f2e0001", "name": "Morning rush", "days_of_week": [0,1,2,3,4],
  "start_time": "07:30", "end_time": "09:00", "surcharge_percent": 20, "trip_types": ["regular","box"],
  "zone_ids": [], "valid_from": null, "valid_to": null, "enabled": true,
  "created_at": "2026-09-25T13:00:00Z", "updated_at": "2026-09-25T13:00:00Z" }
```
- Times are `"HH:MM"` (24h, no seconds), Baghdad local; `end_time` < `start_time` = an **overnight** window that belongs to the day it starts on (Thu 22:00-02:00 covers Fri 00:00-02:00). `days_of_week` is stored sorted.
- Dates are `"YYYY-MM-DD"` or `null` (inclusive, compared with the day the window starts). `zone_ids` empty = all zones; otherwise the rule applies only when the **pickup** is inside one of them.

`PeakRuleInput` = `PeakRule` without `id`, `created_at`, `updated_at`. Omitted optional fields default to `trip_types: ["regular","box"]`, `zone_ids: []`, dates `null`, `enabled: true`; an empty-string date is read as `null`. Any invalid field → `400 {"error":"invalid_peak_rule: <field>"}` naming the first failing field, in this order:

| `<field>` | Rule |
|-|-|
| `name` | trimmed 1..80 characters, no control characters |
| `days_of_week` | non-empty, unique, each 0..6 |
| `start_time` | `"HH:MM"` 24h |
| `end_time` | `"HH:MM"` 24h and not equal to `start_time` |
| `surcharge_percent` | integer 1..200 |
| `trip_types` | non-empty subset of `"regular"`, `"box"` |
| `valid_from` / `valid_to` | `"YYYY-MM-DD"` or null; `valid_to` also when before `valid_from` |
| `zone_ids` | every id must be an existing zone (checked last) |

A body that is not JSON of this shape (missing `name`, a string day, ...) is the framework's `422`.

`PeakPreview` (`at` defaults to now, `trip_type` to `regular`; without `lat`+`lng` zone-limited rules never match):
```json
{ "peak_enabled": true, "max_percent": 50, "peak_percent": 20,
  "rule": { "...": "PeakRule of the winner, null when peak_percent is 0" },
  "matched_rule_ids": ["0a640001-5c7b-4e2a-9d31-6b0c1f2e0001"] }
```
`matched_rule_ids` lists every enabled rule matching that moment, **even when `peak_enabled` is false** (then `peak_percent` is 0 and `rule` null). Bad params → `400 {"error":"invalid_peak_preview: <param>"}` (`at` not RFC3339, `trip_type` not regular/box, `lat`/`lng` out of range or only one sent). URL-encode the `+` of an offset (`%2B`), or send UTC (`...Z`).

**Settings** (existing `PUT /api/admin/settings/{key}` `{"value": "..."}`, super_admin; out of range → 400):

| Key | Seeded | Allowed |
|-|-|-|
| `pricing.peak_enabled` | `"true"` | `"true"` \| `"false"` -- master switch |
| `pricing.peak_max_percent` | `"50"` | integer 0..200 -- cap on any rule |
| `pricing.use_road_distance` | `"true"` | `"true"` \| `"false"` -- road vs straight-line distance |
