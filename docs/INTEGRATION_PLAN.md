...
- ~~**OSRM** is the public demo server (rate-limited, no SLA). Confirm production routing source.~~ **RESOLVED 2026-09-25** — the route line now comes from the backend's own OSRM through `GET /api/routes/driving` (any logged-in JWT; `services/routing.ts`), the same router the fare is priced on. The apps no longer call a routing server directly.
